...
module NavbarHelper
  ...
  def menu_group(options = {}, &block)
    pull_class = "pull-#{options[:pull]}" if options[:pull].present?
  ...
  def menu_item_nested(name, path = "#", *args)
    options = args.extract_options!
    content_tag :li, class: nested_active?(path) do
      link_to name, path, options
    end
  end
  ...
  def menu_text(text = nil, options = {}, &_block)
  ...
  def nav_bar_div(options, &_block)
  ...
  def nested_active?(path)
    "active" if request.fullpath.start_with?(path)
  end
...
class Hash
  ...
  def append_merge!(key, value)
    ...
    self[key] = value && return if current_value.blank?
    # raise error if we're trying to merge into something that isn't a string
    fail ArgumentError, "Can only merge strings" unless current_value.is_a?(String)
